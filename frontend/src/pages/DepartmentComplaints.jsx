...
import { onAdminComplaintUpdate, offAdminComplaintUpdate } from '../services/socketService';
...
export const DepartmentComplaints = () => {
  ...
  const activeCategoryId = adminCategory || categoryId;
  ...
  useEffect(() => {
    ...
    if (categoryId !== adminCategory) {
      navigate(`/admin/department/${adminCategory}`, { replace: true });
      return;
    }
    ...
    fetchData();
  }, [categoryId, adminCategory]);

  useEffect(() => {
    onAdminComplaintUpdate(() => {
      fetchData();
    });

    return () => {
      offAdminComplaintUpdate();
    };
  }, [activeCategoryId]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [complaintRes, teamsRes, wardRes] = await Promise.all([
        adminService.getDepartmentComplaints(activeCategoryId),
        setupService.getTeamsByCategory(activeCategoryId),
        setupService.getAllWardOffices()
      ]);

      setComplaints(complaintRes.data.complaints);
      setStats(complaintRes.data.stats);
      setTeams([...(teamsRes.data.teams || [])].sort((a, b) => a.teamName.localeCompare(b.teamName)));
  ...
  const exportData = () => {
    ...
    a.href = url;
    a.download = `${activeCategoryId}-complaints.csv`;
    a.click();
            ...
            <div className="bg-gray-800 rounded-2xl shadow-2xl max-w-md w-full">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  {teams.length > 0 && (
                    <div className="mb-3">
                      <p className="text-xs uppercase tracking-wide text-gray-400 mb-2">Available Teams</p>
                      <div className="flex flex-wrap gap-2">
                        {teams.map((team) => (
                          <button
                            key={team._id}
                            type="button"
                            onClick={() => setActionData({ ...actionData, teamId: team._id })}
                            className={`rounded-lg px-3 py-2 text-sm border transition-all ${
                              actionData.teamId === team._id
